...
par(mfrow = c(1, 2))
plot(x = X[, sim_data$j[1]], Y, xlab = paste0('X_', sim_data$j[1]))
plot(x = X[, sim_data$j[2]], Y, xlab = paste0('X_', sim_data$j[2]))
```
...
```{r SDF}
fit <- SDForest(Y ~ ., train_data, mc.cores = 22)
fit
```
...
dep1 <- partDependence(fit, sim_data$j[1], mc.cores = 11)
plot(dep1)

dep2 <- partDependence(fit, sim_data$j[2], mc.cores = 11)
plot(dep2)
```
